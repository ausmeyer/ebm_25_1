...
age <- seq(2, 20, by = 0.1)
mean_weight <- 2.5 + 2.3 * age + 0.01 * age^2
sd_weight <- 1.5 + 0.3 * age

# Create percentile curves
p3 <- mean_weight - 2 * sd_weight
p16 <- mean_weight - 1 * sd_weight
p50 <- mean_weight
p84 <- mean_weight + 1 * sd_weight
p97 <- mean_weight + 2 * sd_weight
...
# Calculate 5th percentile for age 11
age_11_mean <- 2.5 + 2.3 * 11 + 0.01 * 11^2
age_11_sd <- 1.5 + 0.3 * 11
p5_weight <- age_11_mean + qnorm(0.05) * age_11_sd

# Create the plot
ggplot(growth_data, aes(x = age)) +
  geom_line(aes(y = p3), color = "#808080", size = 1) +
  geom_line(aes(y = p16), color = "#808080", size = 1) +
  geom_line(aes(y = p50), color = "#43418A", size = 1.5) +
  geom_line(aes(y = p84), color = "#808080", size = 1) +
  geom_line(aes(y = p97), color = "#808080", size = 1) +
  
  # Add labels
  annotate("text", x = 20.5, y = p50[length(p50)], label = "50th (Mean)", 
           color = "#43418A", fontface = "bold", hjust = 0) +
  annotate("text", x = 20.5, y = p84[length(p84)], label = "84th (+1 SD)", 
           color = "#808080", hjust = 0) +
  annotate("text", x = 20.5, y = p16[length(p16)], label = "16th (-1 SD)", 
           color = "#808080", hjust = 0) +
  annotate("text", x = 20.5, y = p97[length(p97)], label = "97th (+2 SD)", 
           color = "#808080", hjust = 0) +
  annotate("text", x = 20.5, y = p3[length(p3)], label = "3rd (-2 SD)", 
           color = "#808080", hjust = 0) +
  
  # Add the boy's data point
  geom_point(x = 11, y = p5_weight, color = "#E74C3C", size = 5) +
  annotate("text", x = 11.5, y = p5_weight, label = "Boy's Weight\n(5th percentile)", 
           color = "#E74C3C", fontface = "bold", hjust = 0) +
  
  labs(title = "Weight-for-Age Growth Chart for Boys",
       x = "Age (Years)",
       y = "Weight (kg)") +
  scale_x_continuous(breaks = seq(2, 20, by = 2), limits = c(2, 22)) +
  theme_minimal() +
...
**Sensitivity focuses only on the "Has AKI" column**
...
set.seed(456)
n <- 200
thumb_sucking <- runif(n, 0, 36)
# Add negative correlation with some noise
food_allergies <- 5 - 0.12 * thumb_sucking + rnorm(n, 0, 0.8)
# Ensure non-negative values
food_allergies[food_allergies < 0] <- runif(sum(food_allergies < 0), 0, 0.5)
...
ggplot(scatter_data, aes(x = thumb_sucking, y = food_allergies)) +
  geom_point(color = "#8CB3D9", alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "#43418A", fill = "#43418A", alpha = 0.2) +
  labs(title = "Relationship Between Thumb-Sucking and Food Allergies",
       x = "Months of Thumb-Sucking",
       y = "Number of Diagnosed Food Allergies") +
  theme_minimal() +
  theme(text = element_text(size = 14),
        plot.title = element_text(size = 18, face = "bold", color = "#43418A"),
        axis.title = element_text(size = 14, face = "bold"))
